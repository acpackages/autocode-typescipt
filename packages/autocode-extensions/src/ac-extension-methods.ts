...
export function fileFromBase64(fileBase64String: string, fileName: string): File {
    let arr: any = fileBase64String.split(','),
        mime = arr[0].match(/:(.*?);/)[1],
...
export function numberRound(number: any, decimals: number = 0): number {
    let result: number = 0;
    try {
...
export function objectFilter(value:any,filterFunction: (key: string, value: any) => boolean): Object {
