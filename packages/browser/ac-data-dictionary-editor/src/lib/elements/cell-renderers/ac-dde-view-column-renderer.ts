...
import { IAcDDEViewColumn } from "../../_ac-data-dictionary-editor.export";


export class AcDDEViewColumnRenderer implements IAcDatagridCellRenderer {
  element?: HTMLElement;
  destroy?(): void {
    if (this.element) {
      this.element.remove();
  ...
  init(args: IAcDatagridCellElementArgs): void {
    ...
    if (value) {
      const viewColumns: IAcDDEViewColumn[] = editorApi.dataStorage.getViewColumns({ columnId: value });
      if (viewColumns.length > 0) {
        element.innerHTML = viewColumns[0].columnName!;
      }
    }
    if (this.element) {
