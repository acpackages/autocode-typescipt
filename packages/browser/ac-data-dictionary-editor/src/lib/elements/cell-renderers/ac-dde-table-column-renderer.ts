...
import { IAcDDETableColumn } from "../../_ac-data-dictionary-editor.export";

export class AcDDETableColumnRenderer implements IAcDatagridCellRenderer {
  element?: HTMLElement;
  destroy?(): void {
    if (this.element) {
      this.element.remove();
  ...
  init(args: IAcDatagridCellElementArgs): void {
    ...
    if (value) {
      const columns: IAcDDETableColumn[] = editorApi.dataStorage.getTableColumns({ columnId: value });
      if (columns.length > 0) {
        element.innerHTML = columns[0].columnName!;
      }
    }
    if (this.element) {
