...
import { AcDDEApi, AcDDECssClassName, AcDDERelationshipRowKey, AcDDETableColumnRowKey, AcDDETriggerRowKey, AcEnumDDEEvent, AcEnumDDEHook, IAcDDERelationshipRow, IAcDDETableColumnRow, IAcDDETableRow, IAcDDETriggerRow } from "../../_ac-data-dictionary-editor.export";
import { acAddClassToElement, AcEnumDatagridEvent, AcEnumResizableEvent, AcEnumResizePanelDirection, AcResizableAttributeName, AcResizablePanels, IAcDatagridRowEvent, IAcResizablePanelResizeEvent } from '@autocode-ts/ac-browser';
...
import { IAcDatagridStateChangeEvent } from "packages/browser/ac-browser/src/lib/ac-datagrid/interfaces/event-args/ac-datagrid-state-change-event.interface";

/* eslint-disable @typescript-eslint/no-non-null-assertion */
export class AcDDEDatagridEditor {
  ...
  state: IAcDDEDatagridEditorState = {};
  ...
  constructor({ editorApi }: { editorApi: AcDDEApi }) {
    ...
    this.tablesDatagrid.hooks.subscribe({
      hookName: AcEnumDDEHook.DatagridActiveTableChange, callback: () => {
    ...
    this.tablesDatagrid.datagridApi.on({
      eventName: AcEnumDatagridEvent.StateChange, callback: (args: IAcDatagridStateChangeEvent) => {
    ...
    this.tableRelationshipsDatagrid.ddeDatagrid.newRowDataFunction = () => {
      ...
      if (this.tablesDatagrid && this.tablesDatagrid.datagridApi && this.tablesDatagrid.datagridApi.activeDatagridRow) {
        const activeRow: IAcDDETableRow = this.tablesDatagrid.datagridApi.activeDatagridRow.data;
        row[AcDDERelationshipRowKey.destinationTableId] = activeRow.table_id;
      }
      return row;
    };
    ...
    this.tableTriggersDatagrid.ddeDatagrid.newRowDataFunction = () => {
      ...
      if (this.tablesDatagrid && this.tablesDatagrid.datagridApi && this.tablesDatagrid.datagridApi.activeDatagridRow) {
        const activeRow: IAcDDETableRow = this.tablesDatagrid.datagridApi.activeDatagridRow.data;
        row[AcDDETriggerRowKey.tableId] = activeRow.table_id;
      }
      return row;
    };
    ...
    this.editorApi.on({
      eventName: AcEnumDDEEvent.StateChange, callback: () => {
  ...
  private initElement() {
    ...
    this.detailPanels.on({
      eventName: AcEnumResizableEvent.resize, callback: (args: IAcResizablePanelResizeEvent) => {
        this.updateEditorState();
      }
    });
  ...
  refreshEditorState() {
    const state = this.editorApi.editorState.datagridEditorState;
    if (state) {
  ...
  updateEditorState() {
    if (this.editorInitialized) {
      ...
      this.editorApi.editorState.datagridEditorState = { ...this.state };
    }
  }
}
