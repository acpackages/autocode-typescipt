...
export class AcRepeaterApi{
  ...
  set usePagination(value: boolean) {
    ...
    this.hooks.execute({ hookName: AcEnumRepeaterHook.UsePaginationChange, args: hookArgs });
  ...
  addRow({ data, append = true, highlightRow = false }: { data?: any, append?: boolean, highlightRow?: boolean } = {}) {
    ...
    this.hooks.execute({ hookName: AcEnumRepeaterHook.RepeaterRowCreated, args: hookArgs });
    ...
    this.hooks.execute({ hookName: AcEnumRepeaterHook.RowAdd, args: addHookArgs });
    const eventArgs: IAcRepeaterRowEvent = {
      repeaterApi: this,
      repeaterRow: repeaterRow
    };
    this.events.execute({ eventName: AcEnumRepeaterEvent.RowAdd, args: eventArgs });
  ...
  deleteRow({ data, rowId, key, value, highlightRow = false }: { data?: any, rowId?: string, key?: string, value?: any, highlightRow?: boolean }) {
    ...
    if (repeaterRow) {
      ...
      this.hooks.execute({ hookName: AcEnumRepeaterHook.RowDelete, args: deleteHookArgs });
      const eventArgs: IAcRepeaterRowEvent = {
        repeaterApi: this,
        repeaterRow: repeaterRow
      };
      this.events.execute({ eventName: AcEnumRepeaterEvent.RowDelete, args: eventArgs });
      this.dataSource.totalRows--;
    }
  }
  ...
  enableExtension({ extensionName }: { extensionName: string }): AcRepeaterExtension | null {
    if (AcRepeaterExtensionManager.hasExtension({ extensionName: extensionName })) {
      ...
      if (extensionInstance) {
        ...
        this.hooks.execute({ hookName: AcEnumRepeaterHook.ExtensionEnabled, args: hookArgs });
  ...
  focusRow({ index, highlightRow = false }: { index: number, highlightRow?: boolean }) {
    ...
    this.hooks.execute({ hookName: AcEnumRepeaterHook.RowFocus, args: hookArgs });
  }
  ...
  setDataSourceType({ dataSourceType }: { dataSourceType: AcEnumDataSourceType }) {
    ...
    this.dataSource.events.subscribe({
      eventName: AcEnumRepeaterEvent.TotalRowsChange,
    ...
    this.hooks.execute({ hookName: AcEnumRepeaterHook.DataSourceTypeChange, args: hookArgs });
  ...
  updateRowPosition({ repeaterRow, oldRepeaterRow }: { repeaterRow: AcRepeaterRow, oldRepeaterRow: AcRepeaterRow }) {
    ...
    this.events.execute({ eventName: AcEnumRepeaterEvent.RowPositionChange, args: eventArgs });
  }
  ...
  updateRow({ data, value, key, rowId, highlightRow = true, addIfMissing = false }: { data: any, value?: any, key?: string, rowId?: string, highlightRow?: boolean, addIfMissing?: boolean }) {
    ...
    if (repeaterRow) {
      ...
      this.hooks.execute({ hookName: AcEnumRepeaterHook.RowUpdate, args: updateHookArgs });
      const eventArgs:IAcRepeaterRowEvent = {
      repeaterApi:this,
      repeaterRow:repeaterRow
    };
    this.events.execute({eventName:AcEnumRepeaterEvent.RowUpdate,args:eventArgs});
    }
    else {
