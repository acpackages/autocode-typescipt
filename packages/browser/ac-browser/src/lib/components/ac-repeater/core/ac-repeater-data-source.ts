...
export class AcRepeaterDataSource {
  ...
  set displayedRepeaterRows(value: AcRepeaterRow[]) {
    ...
    this.repeaterApi.hooks.execute({ hook: AcEnumRepeaterHook.DisplayedRowsChange, args: eventArgs });
    this.events.execute({ event: AcEnumRepeaterEvent.DisplayedRowsChange, args: eventArgs });
  }
  ...
  set totalRows(value: number) {
    ...
    this.repeaterApi.hooks.execute({ hook: AcEnumRepeaterHook.TotalRowsChange, args: eventArgs });
    this.repeaterApi.hooks.execute({ hook: AcEnumRepeaterHook.DisplayedRowsChange, args: eventArgs });
  ...
  setData({ data,startIndex,totalCount }: { data: any[],startIndex?:number,totalCount?:number }) {
    ...
    this.repeaterApi.hooks.execute({ hook: AcEnumRepeaterHook.BeforeDataChange, args: hookArgs });
    this.data = data;
    let index = 0;
    this.repeaterRows = [];
