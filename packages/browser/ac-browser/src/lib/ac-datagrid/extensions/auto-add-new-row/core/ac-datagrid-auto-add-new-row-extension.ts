...
export class AcDatagridAutoAddNewRowExtension extends AcDatagridExtension {
  ...
  set autoAddNewRow(value:boolean){
    ...
    this.datagridApi.hooks.execute({hookName:AcEnumDatagridAutoAddNewRowHook.AutoAddNewRowValueChange,args:hookArgs});
  }

  override handleHook({ hookName, hookArgs }: { hookName: string; hookArgs: any; }): void {
    if(this.autoAddNewRow) {
      if(hookName == AcEnumDatagridHook.CellValueChange){
