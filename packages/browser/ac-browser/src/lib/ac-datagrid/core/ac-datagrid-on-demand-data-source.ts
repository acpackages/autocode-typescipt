  ...
  override getData({ startIndex = 0, rowsCount = 100 }: { startIndex?: number; rowsCount?: number; } = {}): void {
    if (this.onDemandFunction) {
      const successCallback: Function = (response: IAcDatagridOnDemandResponseArgs) => {
        ...
        this.datagridApi.hooks.execute({ hookName: AcEnumDatagridHook.GetOnDemandDataSuccessCallback, args: hookArgs });
      }
      ...
      this.datagridApi.hooks.execute({ hookName: AcEnumDatagridHook.BeforeGetOnDemandData, args: hookArgs });
