import type { IAcDataGridEvent, IAcDataGridGridEvent } from './events';
...
export class AcDGLocalEventService<TEventType extends string> implements IEventEmitter<TEventType> {
    ...
    public dispatchEvent(event: IAcDataGridEvent<TEventType>): void {
        const agEvent = event as IAcDataGridGridEvent<any, any, TEventType>;
    ...
    public dispatchEventOnce(event: IAcDataGridEvent<TEventType>): void {
        if (!this.firedEvents[event.type]) {
            this.dispatchEvent(event);
        }
    }

    private dispatchToListeners(event: IAcDataGridGridEvent<any, any, TEventType>, async: boolean) {
