...
export class AcPaginationComponent extends AcBase implements OnChanges {
  ...
  @Input() set dataController(value: AcDataController) {
    ...
    this._dataController.on('dataFiltered',()=>{
      console.log(this);
  ...
  setActivePage(newActivePage:number){
    ...
    this.setPages();
    let eventData:IAcPaginationEvent = {
    ...
    this.events.execute("pageChange",eventData);
    this.onPageChange.emit(eventData);
    let startIndex: number = this.resultStartNumber - 1;
    let endIndex: number = this.resultEndNumber - 1;
  ...
  setPages() {
    ...
    if(this.pagesDisplayCount < this.totalPages){
      pagesArray = [1];
      let pagesToAdjust: number = this.pagesDisplayCount - 4;
      let pageMedian: number = Math.floor(pagesToAdjust / 2);
    ...
    for (let pageNumber of pagesArray) {
