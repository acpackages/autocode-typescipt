...
export class AcDataController {
  ...
  set data(value: any[]) {
    ...
    this.events.execute("dataChanged");
  ...
  applyDataFilter(): void {
    ...
    if(foundFilters){
      ...
      this.events.execute("dataFiltered");
    }
    else{
  ...
  applyDataSort(): void {
    ...
    if(this.sortOrder && Object.keys(this.sortOrder).length > 0){
      let dataToSort: any[] = [...this.dataFiltered];
      ...
      this.events.execute("dataSorted");
    }
    else{
  ...
  checkRecordValidForFilterCondition(record: any, condition: IAcFilterCondition): boolean {
    ...
    let operator: AcEnumFilterComparisonOperator = condition.operator;
    let conditionValue = condition.value;
    let fieldValue = record[condition.field];
  ...
  checkRecordValidForFilterConditionGroup(record: any, filterGroup: IAcFilterGroup): boolean {
    ...
    let conditions: IAcFilterCondition[] = filterGroup.conditions ?? [];
  ...
  on(eventName:string,fun:Function){
    return this.events.register(eventName,fun);
  }
  ...
  setDisplayData() {
    ...
    if(startIndex !=0 && endIndex != this.dataSorted.length){
      ...
      this.events.execute("displayDataChange");
    }
  }
