...
export class AcDatagridComponent extends AcBaseRepeater{
  ...
  editCell(rowIndex:number,columnIndex:number){
    if(this.rows[rowIndex]){
      let rowInstance:AcDatagridRowComponent = this.rows[rowIndex];
      if(rowInstance.cells[columnIndex]){
        this.editingColumnIndex = columnIndex;
        this.editingRowIndex = rowIndex;
        let cellInstance:AcDatagridCellComponent = rowInstance.cells[columnIndex];
  ...
  getFilterFields():IAcFilterField[]{
    let filterFields:IAcFilterField[] = [];
  ...
  handleCellSelectionChange(){
    ...
    this.selectedRowData = this.selectedRow!.rowData;
  ...
  handleKeyDown(event:any){
    let keyCode:string = event.code.toUpperCase();
    if(keyCode=="TAB"){
      ...
      if(["ARROWDOWN","ARROWLEFT","ARROWRIGHT","ARROWUP","TAB"].includes(keyCode)){
        ...
        if(keyCode=="ARROWDOWN"){
          ...
          if(this.rows[this.selectedRowIndex]){
            let rowInstance:AcDatagridRowComponent = this.rows[this.selectedRowIndex];
  ...
  setColumnsFromComponents(){
    if(this.columnComponents){
      let index:number = -1;
      for(let column of this.columnComponents){
        index++;
        let columnDetails:IAcDataGridColumn = {
        ...
        if(column.title!=""){
          columnDetails.title = column.title;
        }
  ...
  stopActiveEditCell(){
    if(this.editingColumnIndex >=0 && this.editingRowIndex >=0){
      if(this.rows[this.editingRowIndex]){
        let rowInstance:AcDatagridRowComponent = this.rows[this.editingRowIndex];
        if(rowInstance.cells[this.editingColumnIndex]){
          let cellInstance:AcDatagridCellComponent = rowInstance.cells[this.editingColumnIndex];
